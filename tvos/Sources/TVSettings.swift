...
// TVSettings - this Apple TV's own preferences.
//
// UserDefaults is still written on every change, but it is now a CACHE. Since
// Phase 2 the source of truth is the device registry on FrameServer, so these
// values can also be read and changed from the hub dashboard on a laptop -
// without giving up the on-screen picker, which writes to the hub like any
// other client. See TVSettingsHubSync.swift.
//
// Still deliberately separate from AppModel/AppSettings, which mirrors the
// kitchen's house-wide settings via /api/state. These values are TV-specific;
// the kitchen and the TV are allowed to diverge here.
//
// Fields:
...
final class TVSettings: ObservableObject {
    ...
    init() {
        ...
        // Everything above has loaded from UserDefaults, so the hub sees this
        // TV's real values on first contact rather than a half-built object.
        startHubSync()
    }

    // MARK: - Hub sync (see TVSettingsHubSync.swift)

    /// Debounce timer for outbound PATCHes; cancelled and restarted on each change.
    var hubPatchTask: Task<Void, Never>?
    /// Fires a PATCH whenever any @Published property changes.
    var hubChangeSink: AnyCancellable?
    /// Suppresses the outbound PATCH while applying a payload we just received,
    /// so a poll cannot echo straight back to the hub.
    var applyingFromHub = false
